...
- **サブスクリプション**: 要明確化（スマホアプリのプロトコルとエンドポイント）
  - オプション: Email, SMS, Application endpoint (Mobile Push)
  - 推奨: 初期実装ではEmailで検証し、後続でMobile Push対応
...
1. **スマホアプリの通知プロトコル**
   - Email (初期実装推奨)
   - SMS
   - Mobile Push (AWS SNS Mobile Push)
   - カスタムWebhook

2. **アラート送信先のエンドポイント**
   - 実際のメールアドレスまたはアプリエンドポイント
   - dev環境とstaging環境で異なる値

3. **予算額の環境ごとの差異**
   - dev: 1000円
   - staging: 1000円（同じ？異なる？）

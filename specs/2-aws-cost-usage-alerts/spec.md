...
システム管理者として、予算アラートの設定をインフラストラクチャコードで管理したい。これにより、設定の再現性を保ち、変更履歴を追跡できる。
...
- AWSアカウント全体のコストを監視する場合、環境別のコスト内訳は必要か？
  → 予算はアカウント全体で1つ設定。環境別の詳細はCost ExplorerまたはタグベースのコストレポートでAWSコンソール上で確認可能。
...
- **FR-008**: 予算とアラートはAWSアカウント単位で設定されなければならない（環境別ではなくアカウント全体のコストを監視）
...
- **Budget（予算）**: AWS利用額の上限を定義するエンティティ
  - budgetName: 予算の名前（例: "attendance-kit-account-monthly-budget"）
  - budgetLimit: 予算額（1000円）
  - timeUnit: 期間単位（MONTHLY）
  - budgetType: 予算タイプ（COST）
  - scope: アカウント全体のコスト（環境別ではない）

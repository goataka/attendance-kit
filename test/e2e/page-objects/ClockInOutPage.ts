...
export class ClockInOutPage {
  ...
  async expectSuccessMessage(expectedText?: string): Promise<void> {
    const messageElement = await this.page.waitForSelector(this.selectors.successMessage, {
      timeout: this.timeouts.waitMessage,
    });
    ...
    if (expectedText) {
      await expect(messageElement).toContainText(expectedText);
    }
  }
  ...
  async expectErrorMessage(expectedText?: string): Promise<void> {
    const messageElement = await this.page.waitForSelector(this.selectors.errorMessage, {
      timeout: this.timeouts.waitMessage,
    });

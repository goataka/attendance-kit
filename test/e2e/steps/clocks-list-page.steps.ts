...
async function createClocksListPage(page: Page) {
  const { ClocksListPage } = await import(
    '@/ClocksListPage/tests/integration/ClocksListPage.page'
  );

  return new ClocksListPage(page);
}
...
Then('打刻一覧を確認する', async function (this: CustomWorld) {
  ...
  const clocksListPage = await createClocksListPage(this.page);

...
- **TSDoc/JSDocの不使用**
  - TSDoc/JSDoc形式のコメント（`/**`で始まるコメント）は原則不要
  - 理由: 関数名や変数名、型情報で意図を明確に表現する
  - 必要な補足説明は通常のコメント（`//`）で十分
...
- **Parameterized Test（パラメータ化テスト）の利用**
  ...
  - TypeScript: `test.each`、`describe.each`、`it.each`を使用
  - シェルスクリプト: `execute_parametrized_test`を使用
  - テストケースのラベル（フィールド名）は日本語で記載する（例: `ケース`、`入力値`、`期待値`）

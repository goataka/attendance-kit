...
1. **現状調査**: 対象ファイルの構造と内容を確認し、どの検証コマンドが必要かを判断する
2. **実行場所の確認**: プロジェクトのルートディレクトリ（`/home/runner/work/attendance-kit/attendance-kit`）で検証コマンドを実行する
3. **スクリプトの利用**: `package.json`に定義されたnpmスクリプトを優先的に使用する
4. **思考の明示**: アクションを起こす前に、なぜそのコマンドが必要か、どのような結果を期待するかを明示する
...
2. **スキル利用の判断記録**:
   ...
   - スキルを使用しなかった場合: その理由（該当スキルがない、または自身で対応可能など）

...
**必須**: 以下の手順で実際のPR差分リンクを生成し、記載してください：
1. `git branch --show-current` で現在のブランチ名を取得
2. `github-mcp-server-search_pull_requests` でPR番号を取得（query: "head:<ブランチ名> is:open"）
3. **`git log --format=%H` で始点・終点の完全なコミットハッシュ（40文字）を実際に取得**
   - コマンドを実際に実行してください（適当な文字列を作成しない）
   - 例: `git log --format=%H -2` で最新2件の完全なハッシュを取得
4. 以下の形式でリンクを生成：`PR差分: [#<PR番号>](https://github.com/goataka/attendance-kit/pull/<PR番号>/changes/<始点コミット>..<終点コミット>)`

詳細は本ドキュメントの「[PR差分リンク提示のルール](#pr差分リンク提示のルール)」を参照。
```

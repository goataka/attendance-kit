...
タスク開始時、以下の手順でスキルの使用を判断してください:

1. **タスクの性質を特定**: 対象ファイル/ディレクトリ、変更の種類を確認
2. **該当スキルの検索**: 下記のスキル一覧から該当するものを探す
3. **判断基準の照合**: 「使用すべき状況」と「判断基準」に照らして該当するか確認
4. **判断結果の記録**: スキルを使用する/しない理由を明示的に記録

**スキル使用が推奨されるケース**:

- 複数のステップ（lint、ビルド、テスト）を伴う開発作業
- 体系的なプロセス（エラー確認→再現→修正→検証）が必要な作業
- リポジトリ全体の検索・更新が必要な作業（ファイルリネームなど）

**スキル不要と判断できるケース**:

- 単一ファイルの軽微な修正（タイポ修正、コメント追加など）
- ドキュメントのみの変更（READMEの更新など）
- 設定ファイルの単純な値変更（環境変数の追加など）
...
- **aws-investigation**: AWS調査
  ...
  - **具体例**:
    - ✅ 使用: デプロイされたスタックの状態確認
    - ✅ 使用: DynamoDBテーブルのデータ確認
    - ✅ 使用: Lambda関数の設定確認
    - ❌ 不要: CDKコードの変更（infrastructure-developerを使用）
...
- **cloudfront-error-handler**: CloudFrontエラー対応
  ...
  - **具体例**:
    - ✅ 使用: CloudFront経由のAPI呼び出しでの403/502エラー
    - ✅ 使用: フロントエンドアプリへのアクセス不具合
    - ✅ 使用: CloudFront-API Gateway連携の問題調査
    - ❌ 不要: ワークフローのエラー（workflow-error-handlerを使用）
...
- **e2e-developer**: E2E開発
  ...
  - **具体例**:
    - ✅ 使用: 新しいE2Eシナリオ（.feature）の追加
    - ✅ 使用: Step定義の実装・修正
    - ❌ 不要: Backendの単体テストの修正
    - ❌ 不要: Frontendのコンポーネントテストの修正

- **file-refactor**: ファイル/フォルダの名称変更・削除と参照更新
  - **使用すべき状況**: ファイルやフォルダのリネーム・削除を依頼された場合
  - **具体例**:
    - ✅ 使用: ファイル名の変更とimport文の一括更新
    - ✅ 使用: ディレクトリのリネームと全参照箇所の更新
    - ✅ 使用: 不要ファイルの削除と依存関係の整理
...
- **frontend-developer**: Frontend開発
  ...
  - **具体例**:
    - ✅ 使用: 新しいReactコンポーネントの作成
    - ✅ 使用: 既存ページのUI/ロジック修正
    - ✅ 使用: スタイリングの変更（CSS/Tailwind）
    - ❌ 不要: Frontend README.mdのタイポ修正のみ
...
- **infrastructure-developer**: Infrastructure開発
  ...
  - **具体例**:
    - ✅ 使用: CDK Constructの追加・修正
    - ✅ 使用: AWSリソース定義の変更
    - ✅ 使用: CloudFormationスタック設定の変更
    - ❌ 不要: Infrastructure README.mdのタイポ修正のみ

- **premerge-check**: プレマージワークフローのローカル実行
  - **使用すべき状況**: PRを作成する前にローカルでCI/CDチェックを実行したい場合
  - **使用しない状況**: 既にCI/CDで検証済みの変更の場合
  - **判断基準**: ローカル環境でCI/CDと同等の検証を実施したい場合は使用
  - **具体例**:
    - ✅ 使用: PR作成前の事前検証
    - ✅ 使用: CI/CDエラーのローカル再現
    - ❌ 不要: 単一ワークスペースの変更のみ（専用スキルを使用）

- **rule-making**: ルール化の依頼対応
  - **使用すべき状況**: ユーザーから「ルール化してください」というリクエストがあった場合
  - **具体例**:
    - ✅ 使用: コーディング規約の追加
    - ✅ 使用: プロジェクト固有のベストプラクティスの文書化
    - ✅ 使用: 命名規則の策定と記録
...
- **workflow-error-handler**: ワークフローのエラー対応
  ...
  - **具体例**:
    - ✅ 使用: premergeワークフローの失敗調査
    - ✅ 使用: デプロイワークフローのエラー対応
    - ✅ 使用: テスト失敗の原因特定と修正
    - ❌ 不要: ワークフローファイルへの環境変数追加のみ
...
- **ドキュメントやテキストのみの変更以外**では、必ずE2Eテストを実行してください
- 「軽微なコード変更」「設定変更」「テストコード変更のみ」の場合でも、ドキュメントやテキストのみの変更に該当しない限りE2Eテストを省略してはいけません
- E2Eテスト未実行のままタスク完了報告をしてはいけません

### スナップショット更新ルール（Frontend）

- Frontend（`apps/frontend/`）に変更がある場合、スナップショット差分が発生したときは必ず更新内容を確認し、差分ファイルをコミットに含めてください
- スナップショット差分があるのに未コミットのまま作業を完了してはいけません
- スナップショット更新時は、意図したUI変更であることを確認してからコミットしてください

### スクリーンショット取得ルール（必須）

- ドキュメントやテキストのみの変更以外では、変更内容に対応する画面のスクリーンショットを少なくとも1枚取得してください
- Frontend（`apps/frontend/`）の変更時は、変更後の画面を必ずスクリーンショットで確認してください
- 完了報告には、取得したスクリーンショットの保存先（パスまたは添付リンク）を必ず含めてください
